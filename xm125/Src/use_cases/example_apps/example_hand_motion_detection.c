...
void hand_motion_detection_set_config(hand_motion_detection_preset_t preset, hand_motion_detection_config_t *config)
{
	switch (preset)
	{
		...
		case HAND_MOTION_DETECTION_PRESET_FAUCET:
			...
			acc_detector_presence_config_frame_rate_app_driven_set(config->presence_config, true);
                                                                    ...
                                                                    acc_sensor_id_t                      sensor_id)
{
	...
	if (status)
	{
		...
		handle->hand_motion_detection_timeout_duration =
			(uint16_t)(roundf(hand_motion_detection_config->hand_detection_timeout *
		...
		handle->detection_retention_duration =
			(uint16_t)(roundf(hand_motion_detection_config->algo_config.detection_retention_duration *
			                  hand_motion_detection_config->algo_config.frame_rate));
	...
	if (status)
	{
		...
		handle->sensor_processing =
			acc_processing_create(handle->hand_motion_detection_sensor_config, &handle->sensor_processing_metadata);
		handle->hand_motion_detection_presence_processor = acc_detector_presence_processing_create(
			handle->hand_motion_detection_presence_processing_config,
	...
	if (status)
	{
		if (handle->config.use_presence_detection)
		{
			...
			uint32_t sleep_time_ms = (uint32_t)(1000.0f / acc_detector_presence_config_frame_rate_get(handle->config.presence_config));
			acc_integration_set_periodic_wakeup(sleep_time_ms);
		}
		else
		{
			uint32_t sleep_time_ms = (uint32_t)(1000.0f / handle->config.algo_config.frame_rate);
			acc_integration_set_periodic_wakeup(sleep_time_ms);
		}
	}
...
bool hand_motion_detection_get_buffer_size(hand_motion_detection_handle_t *handle, uint32_t *buffer_size)
{
	...
	{
		*buffer_size =
			(hand_motion_detection_buffer_size >
			 presence_detector_buffer_size) ? hand_motion_detection_buffer_size : presence_detector_buffer_size;
	}
...
bool hand_motion_detection_prepare(hand_motion_detection_handle_t *handle, acc_sensor_t *sensor, const acc_cal_result_t *cal_result, void *buffer,
                                   ...
                                   bool force_prepare)
{
	...
	if (handle->prepare_needed || force_prepare)
	{
		...
		{
			status = acc_detector_presence_prepare(handle->presence_detector, handle->config.presence_config, sensor, cal_result, buffer,
			                                       buffer_size);
		}
...
void hand_motion_detection_process(hand_motion_detection_handle_t *handle, void *buffer,
                                   hand_motion_detection_result_t *hand_motion_detection_result)
{
	...
	else // HAND_MOTION_DETECTION_APP_MODE_HAND_MOTION
	{
		...
		acc_detector_presence_processing_process(handle->hand_motion_detection_presence_processor, buffer,
...
static void set_sensor_config(const hand_motion_detection_config_t *hand_motion_detection_config, acc_config_t *sensor_config)
{
	...
	{
		int32_t start_point = (int32_t)roundf(
			(algo_config->sensor_to_water_distance - profile_envelope_fwhm_m - water_jet_half_width) / ACC_APPROX_BASE_STEP_LENGTH_M);
	...
	if (profile_envelope_fwhm_m + water_jet_half_width + algo_config->sensor_to_water_distance < algo_config->measurement_range_end)
	{
		...
		uint16_t num_points  =
			(uint16_t)roundf(fmaxf(1.0f,
			                       (algo_config->measurement_range_end - start_m) /
			                       ((float)step_length * ACC_APPROX_BASE_STEP_LENGTH_M)));
