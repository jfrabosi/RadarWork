 ...
 */
static bool handle_indications(hand_motion_detection_handle_t *handle, hand_motion_detection_result_t *result, acc_sensor_t *sensor,
                               acc_cal_result_t *cal_result,
                               void *buffer, uint32_t buffer_size, bool *data_reliable);
...
int acconeer_main(int argc, char *argv[])
{
	...
	while (true)
	{
		...
		if (!handle_indications(hand_motion_detection_handle, &hand_motion_detection_result, sensor, &cal_result, buffer, buffer_size,
		                        &data_reliable))
		{
